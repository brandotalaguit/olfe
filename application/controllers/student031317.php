...
class Student extends CI_Controller {
    ...
    public function validate_verification_code($verification_code)
    {
        ...
        $no_login_student = $this->db->where('isloggedin',1)->get('tblstudentlogin')->num_rows();

		if($no_login_student >= $evaluation_period->max_users)
		{
    ...
    public function current_load()
    {
        ...
        $islogin = $this->db->where(array('studentlogin_id'=>$studentlogin_id, 'isloggedin'=>1))
                            ->get('tblstudentlogin')->row();

        if(! count($islogin))
        {
            $this->logout('You have been log-out because of 60 minutes of inactivity.');
        }
    ...
    public function evaluate($sched_id = NULL)
    {
		...
		if(isset($_POST['btn_proceed_feedback']))
		{
			...
			redirect("student/feedback/{$sched_id}",'refresh');
		}
